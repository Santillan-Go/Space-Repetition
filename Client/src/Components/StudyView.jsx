...
//let lastReviewDate=focus.lastReviewDate;
  const nextTime=programNextReview({level,lastReviewDate:focus.lastReviewDate})
  //JUST FOR GOOD
  if(level==="good"){
  ...
  let UpdataCard={front:focus.front,nextTime,id,cardID:focus.id,lastReviewDate}
...
<h2 className='congratulation-cards-text'>!Felidades Has terminado!</h2>}
 ...
 { studyToday.length>0?    <div  className='btns-card'>
     

        { showAnswer? <>
        
          <button className='btn-close-card' onClick={e=>{
        ...
        <button className='btn-good'onClick={e=>{
          showCardFocusOn({id,decks})
          spaceR({level:"good"})
          }} ><svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="32" d="M464 128L240 384l-96-96M144 384l-96-96M368 128L232 284"/></svg></button>
          </>:<button className='show-answer-card' onClick={()=> setAnswer(true)}> Mostar Respuesta</button>
     }
      </div>:<h2 className='congratulation-cards-emoji'>🥰</h2>}
    </section>
  );
}
