...
const result= await FechRequest.getOneUserById(`${URL}/one-user/${inUser._id}`);
setUser(result)
setLoader(false);
    }
GetUserIn()
...
if(!name) return;
console.log(name)
console.log(decks,"I am in FORM")
setDeck(oldstate=>{
...
<section  className={`modal-add ${add?"":"none"}`} >
  ...
  <div className="content-modal"><h2 className="text-center">Crear Mazo</h2>
  
  <form onSubmit={newDeck} className="form-add-modal">
<input type="text"  name="deck" placeholder="Nombre del Mazo" autoComplete="off"/>
